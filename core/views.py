...
def ckeditor_image_upload(request):
    ...
    if upload is None:
        return JsonResponse({'error': {'message': 'Файл табылмады.'}}, status=400)

    if upload.size > MAX_UPLOAD_SIZE:
        return JsonResponse({'error': {'message': 'Файл өлшемі 5MB-дан аспауы керек.'}}, status=400)

    try:
        Image.open(upload).verify()
    except (UnidentifiedImageError, OSError):
        return JsonResponse({'error': {'message': 'Жарамды сурет файлын жүктеңіз.'}}, status=400)
