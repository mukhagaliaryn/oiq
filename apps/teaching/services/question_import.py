...
def _build_question_schema(format_code):
    if format_code == 'matching':
        item_properties = {
            'format_code': {'type': 'string', 'enum': [format_code, 'unsupported']},
            ...
            'pairs': {
                ...
                'items': {
                    'type': 'object',
                    'properties': {
                        'left_html': {'type': 'string'},
                        'right_html': {'type': 'string'},
                    },
                    'required': ['left_html', 'right_html'],
                    'additionalProperties': False,
                },
            },
            'warning': {'type': 'string'},
        }
        required = ['format_code', 'level', 'text_html', 'pairs', 'warning']
    else:
        item_properties = {
            'format_code': {'type': 'string', 'enum': [format_code, 'unsupported']},
            'variant_code': {'type': 'string', 'enum': ['single', 'multiple']},
            'level': {'type': 'string', 'enum': ['easy', 'medium', 'hard']},
            'text_html': {'type': 'string'},
            'options': {
                'type': 'array',
...
_MATCHING_IMPORT_PROMPT_RULES = """\
3. "format_code": use "matching" only for matching-type questions that present two columns/lists of items \
   where each item on the left must be paired with exactly one item on the right (сәйкестендіру). Use \
   "unsupported" for anything else (single/multiple choice, fill-in-the-blank, open-ended/essay questions, \
   etc.) — for those, still fill in "text_html" with the question text and leave "pairs" as an empty array, \
   and explain briefly in "warning" why it was not imported.
4. "pairs": list every left/right pair in the exact order they appear in the source. Each pair's "left_html" \
   and "right_html" must be plain HTML fragments (no <html>/<body> wrapper).
5. "level": infer the difficulty (easy/medium/hard) from the question's complexity. Default to "medium" \
   if you cannot tell.
6. "text_html" must be a plain HTML fragment (no <html>/<body> wrapper). Use \
   <p>, <ul>/<ol>/<li>, and <table>/<tr>/<td> tags to preserve paragraphs, lists, and tables exactly as \
   they are laid out in the source.
"""

_IMPORT_PROMPT_OUTRO = """\
...
8. Images: number every embedded image sequentially across the whole document, starting at 1, in the \
   ...
   insert a placeholder token in that exact form: {{img:N}} — where N is that image's number. Do not \
   describe the image, only place the token.
9. "warning" must be an empty string "" when there is nothing to flag.
10. Do not invent questions or content that are not in the document. Do not skip any question.
"""


def _build_import_prompt(format_code):
    rules = _MATCHING_IMPORT_PROMPT_RULES if format_code == 'matching' else _TEST_IMPORT_PROMPT_RULES
    return _IMPORT_PROMPT_INTRO + rules + _IMPORT_PROMPT_OUTRO
...
class ParsedQuestion:
    ...
    level: str
    text_html: str
    variant_code: str = 'single'
    options: list = field(default_factory=list)
    pairs: list = field(default_factory=list)
    warning: str = ''

    def is_supported_for(self, format_code):
        return self.format_code == format_code
...
def _call_claude(pdf_bytes, format_code):
    ...
    try:
        with client.messages.stream(
            ...
            output_config={
                'effort': 'high',
                'format': {'type': 'json_schema', 'schema': _build_question_schema(format_code)},
            },
            messages=[{
                'role': 'user',
                'content': [
                    ...
                    {'type': 'text', 'text': _build_import_prompt(format_code)},
                ],
            }],
        ) as stream:
...
def run_question_import(docx_bytes, batch_id, format_code):
    pdf_bytes = _convert_docx_to_pdf(docx_bytes)
    images = _extract_ordered_images(docx_bytes)
    payload = _call_claude(pdf_bytes, format_code)
    ...
    for raw in payload.get('questions', []):
        ...
        pairs = [
            ParsedPair(
                left_html=_substitute_image_placeholders(pair.get('left_html', ''), image_urls),
                right_html=_substitute_image_placeholders(pair.get('right_html', ''), image_urls),
            )
